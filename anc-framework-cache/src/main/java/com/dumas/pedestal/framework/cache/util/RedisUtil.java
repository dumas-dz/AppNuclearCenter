...
public final class RedisUtil {
    ...
    public boolean tryLock(String lockKey, int expireTime) {
        ...
        this.expire(lockKey, expireTime);
        return true;
    }
    ...
    public long sUnionAndStore(String key1, String key2, String key3) {
    ...
    public Object doAction(String com, String... args) {
        byte[][] bs = Optional.ofNullable(args)
                .map(ars -> Stream.of(ars).map(ar -> ar.getBytes()).collect(toList()).toArray(new byte[args.length][])).orElse(null);

        return redisTemplate.execute(new RedisCallback<Object>() {
            @Override
            public Object doInRedis(RedisConnection connection) throws DataAccessException {
                return connection.execute(com, bs);
            }
        });
    }
    ...
    public Set<Object> getRedisList(String preKey) {
        Set<Object> keys = redisTemplate.keys(preKey + "*");
        return keys;
    }
